...
function shareText(score) {
    ...
    SubmitResults();
    if (score <= 60) return '等级：LEVEL 1 文化沙漠';
    if (score <= 89) return '等级：LEVEL 2 浅入文化';
    if (score <= 109) return '等级：LEVEL 3 文化小鬼';
    if (score <= 118) return '等级：LEVEL 4 文化才子';
    if (score <= 125) return '等级：LEVEL 5 文化爱人';
    if (score <= 135) return '等级：LEVEL 6 文化战士';
    if (score <= 145) return '等级：LEVEL 7 文化巨人';
    if (score <= 156) return '等级：LEVEL 8 文化先锋';
    if (score <= 173) return '等级：LEVEL 9 文化杀手';
    if (score <= 190) return '等级：LEVEL 10 独爱文化';
    return '等级：LEVEL SSR 文化祖师';
}
